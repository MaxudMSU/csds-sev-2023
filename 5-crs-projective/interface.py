...
def true_start():
    ...
    par_list = matrix_box.get(1.0,END).split()
    iter_num = 0
    times = 0
    os.system('g++ crs-projective.cpp')
    proga_work()
...
def proga_work():
    ...
    global iter_num
    start = time.time()
    current_par_list = ' '.join(par_list) + ' ' + str(h) + ' ' + str(isStart)
...
filemenu.add_command(label='1(сопряжённые)', command=show_matr_1)
filemenu.add_command(label='2(комплексные)', command=show_matr_2)
filemenu.add_command(label='3(разные)', command=show_matr_3)
filemenu.add_command(label='4(разные)', command=show_matr_4)
...
h_entry = Entry(matrix_frame)
h_entry.pack(expand=True, fill=X)

matrix_box = Text(matrix_frame)
matrix_box.pack(expand=True, fill=BOTH)
