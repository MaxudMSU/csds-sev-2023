...
def startuem():
    a = eval(para_entry.get())
    b = eval(parb_entry.get())
    iters = eval(iters_entry.get())
    start = time.time()
    comm_start = f'./a.out {a} {b} {iters} > file.osip'
    #print(comm_start)
    os.system(comm_start)
    timee = time.time() - start
    res4_value.config(text=str(timee))
    show_res()
...
def show_res():
    with open('file.osip') as file:
        raw = file.read()
    # splt = raw.split('\n')
    res_comps = Text(iter_frame, height=4)
    res_comps.pack(side=LEFT, expand=True)

    sb = Scrollbar(iter_frame)
    sb.pack(side=RIGHT, fill=BOTH)

    res_comps.config(yscrollcommand=sb.set)
    sb.config(command=res_comps.yview)

    res_comps.insert('end', raw)

    crm = Label(frame2, text='')
    crm.pack(expand=True, fill=BOTH)
    crm_image = ImageTk.PhotoImage(PILImage.open('image.png'))
    crm.image = crm_image
    crm.config(image=crm_image)
...
window = Tk()
window.title('Спектр усреднения')
...
osn_label = Label(frame111, text='Функция оснащения: ', anchor='e')
osn_label.pack(expand=True, fill=BOTH)

iters_label = Label(frame111, text='Количество итераций: ', anchor='e')
iters_label.pack(expand=True, fill=BOTH)
...
osn_entry = Entry(frame112)
osn_entry.pack(expand=True, fill=X)

iters_entry = Entry(frame112)
iters_entry.pack(expand=True, fill=X)
...
start_btn = Button(frame2, text='Посчитать', command=startuem)
start_btn.pack(expand=True, pady=10)

iter_btn = Button(frame2, text='Показать ЦРМ', command=None)
iter_btn.pack(expand=True, pady=10)
...
entery_list = [x_entry,y_entry,x1_entry, y1_entry, x2_entry, y2_entry, para_entry, parb_entry, iters_entry, osn_entry]
# res_entries = [res1_value, res5_value, res3_value, res4_value, res6_value]
res_entries = [res4_value]
